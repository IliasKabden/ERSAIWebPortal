function AppViewModel(dataModel) {
    ...
    var PathNames = ["/Account/Login", "/Account/GetPassword"];
    self.initialize = function () {
        var fragment = common.getFragment();
        if ((!dataModel.getAccessToken() && PathNames.indexOf(window.location.pathname) < 0) || fragment.access_token) {
            ...
            if (fragment.access_token) {
                ...
                if (localStorage.PathName)
                    window.location = localStorage.PathName;
            }
            else {
