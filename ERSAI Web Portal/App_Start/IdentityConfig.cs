...
namespace ERSAI_Web_Portal
{
    ...
    public class ApplicationUserManager : UserManager<AppUser, string>
    {
        public ApplicationUserManager(IUserStore<AppUser,string> store)
            : base(store)
        {
            MvcApplication.Current.ASPIdentityPasswordHasher = PasswordHasher;
        }
        public static ApplicationUserManager Create(IdentityFactoryOptions<ApplicationUserManager> options,
            IOwinContext context)
        {
            var manager = new ApplicationUserManager(new UserStore<AppUser, AppRole, string, AppUserLogin, AppUserRole, AppUserClaim>(context.Get<ERSAIContext>()));
            ...
            if (dataProtectionProvider != null)
            {
                manager.UserTokenProvider =
                    new DataProtectorTokenProvider<AppUser>(dataProtectionProvider.Create("ERSAI Web Portal"));
            }
            return manager;
        }
    }
